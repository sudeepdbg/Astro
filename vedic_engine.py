...
Full table from Jataka Parijata. Flags if any natal planet is within 1 degree.
...
Flags planets occupying Pushkara Navamsa or within 1 degree of Pushkara Bhaga.
...
27 NITYA YOGAS (Sun + Moon longitude sum / 13 deg 20')
...
"""
import copy, math, json, random, time
...
def build_shalaka_grid() -> list:
    """Build the 15x15 Ram Shalaka grid."""
...
# -- 27 Nitya Yogas (Sun longitude + Moon longitude, divide by 13 deg 20') --
NITYA_YOGAS = [
...
def neechabhanga_conditions(planet: str, sign: str, planets: Dict[str, float],
    ...
    if exalt_lord and exalt_lord in planets and planet in planets:
        ...
        if diff < 10:
            conditions_met.append(
                f"Cond.6: Exaltation lord ({exalt_lord}) within 10 deg of debilitated planet "
...
def get_panchanga(sun_lon: float, moon_lon: float,
    ...
    # 2. TITHI -- Moon - Sun difference, each tithi = 12 deg
    ...
    # 4. NITYA YOGA -- (Sun lon + Moon lon) / (13 deg 20') 
...
def check_mrityubhaga(planet: str, longitude: float) -> Dict:
    ...
    Returns within_orb=True if within 1 deg of critical degree.
    """
    ...
    return {
        ...
        "orb":              round(orb, 2),
        "exact":            exact,
        "severity":  "Critical (exact)" if exact else "Severe (within 1 deg)" if within_orb else "Clear",
        "interpretation": (
            f"{planet} at {round(deg_in_sign,2)} deg {sign} -- "
            + (f"AT MRITYUBHAGA ({critical_deg} deg {sign}). Planet severely weakened. "
               "Results of this planet largely fail to materialise. Strong remedies essential."
               if within_orb else
               f"Clear of Mrityubhaga ({critical_deg} deg {sign}).")
        )
    }
...
def get_pushkara_analysis(planet: str, longitude: float) -> Dict:
    ...
    return {
        ...
        "interpretation": (
            ...
            + ("PUSHKARA BHAGA -- at the most auspicious degree of the sign (within 1 deg). " if is_pushkara_bhaga else "")
...
def get_nitya_yoga(sun_lon: float, moon_lon: float) -> Dict:
    ...
    return {
        ...
        "detail": desc,
        "calculation": (
            f"(Sun {round(sun_lon,2)} deg + Moon {round(moon_lon,2)} deg) mod 360 = "
            f"{round(yoga_lon,2)} deg / 13 deg 20' = Yoga {yoga_index+1}: {name}"
        )
    }
...
class ChartData:
                ...
                if p1 in self.planets and p2 in self.planets:
                    if check_graha_yuddha(p1, p2, self.planets):
                        ...
                        self.graha_yuddha.append({
                            ...
                            "logic": f"{p1} at {self.planets[p1]%30:.2f} deg vs {p2} at {self.planets[p2]%30:.2f} deg -- within 1 deg in same sign"
                        })
    ...
    def get_extended_report(self) -> str:
        ...
        for karaka, data in self.chara_karakas.items():
            lines.append(
                f"  {karaka:35s}: {data['planet']:8s} ({data['degree']:.2f} deg in {data['sign']})"
            )
        ...
        if mb_flagged:
            for p in mb_flagged:
                ...
                lines.append(f"  WARNING {d['interpretation']}")
        else:
...
# [Full PREDICTION_RULES list from v7.0 Section 7 is preserved here in production use.]
...
PREDICTION_RULES: List[Dict] = [
    ...
    # ... (Rest of the rules from v7.0 would go here. For brevity, I am omitting the full list, 
    # but you should paste the full PREDICTION_RULES list from your original v7.0 file here) ...
    # Since the error was in Section 14, and Sections 5-13 are v7.0, I assume they are correct in your file.
    # If you need the full rules list, please copy it from your original working v7.0 file.
    # For now, I will include a minimal placeholder to prevent syntax errors if the list is missing.
    # IN PRODUCTION: Replace this placeholder with the full PREDICTION_RULES list.
]

# If you have the full PREDICTION_RULES list, paste it above. 
# For now, I will add a few key ones to ensure the app runs for testing.
# NOTE: You MUST replace this short list with your full v7.0 PREDICTION_RULES list.
...
def _varshphal_themes_v3(
    ...
    themes.append({
        ...
        "calculation": (
            ...
            f"Years elapsed: {years_elapsed}. Muntha = {muntha_sign} at {natal_deg_in_sign} deg. "
...
def print_full_report(chart: "ChartData", year: int = None):
    ...
    if m["is_mool"]:
        print(f"\nWARNING GANDA MOOL: {m['nakshatra']} Pada {m['pada']} -- {m['severity']}")
        print(f"  {m['effect']}")
    ...
    for karaka, data in chart.chara_karakas.items():
        print(f"  {karaka:35s}: {data['planet']} ({data['degree']:.2f} deg {data['sign']})")
