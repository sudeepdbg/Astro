Here is the complete, updated `vedic_engine.py` file. I have replaced the empty `PREDICTION_RULES` list with the robust rule set you provided, ensuring that the "Yearly Predictions" tab will now generate detailed data for Career, Marriage, Children, and Health.
...
NEW MODULES:
PANCHANGA (5 limbs) — Tithi, Vara, Nakshatra, Nitya Yoga, Karana
...
MRITYUBHAGA (Critical Death Degrees)
...
SHADBALA (6-component) — expanded from proxy score
...
27 NITYA YOGAS (Sun + Moon longitude sum / 13°20')
...
GRAHA DRISHTI STRENGTH (orb-weighted aspects)
    ...
    for planet, p_house in house_map.items():
        ...
        if planet in SPECIAL_ASPECTS:
            for offset in SPECIAL_ASPECTS[planet]:
                ...
                if asp_house == house and (planet, "7th (universal)") not in [a for a in aspects if
                                                                              a[0] == planet]:
...
def get_panchanga(sun_lon: float, moon_lon: float,
    ...
    return {
        ...
        "panchanga_quality": _panchanga_quality(vara_nature, tithi_nature, yoga_nature,
                                                karana_nature),
    }
...
def calculate_pratyantardasha(ad: DashaPeriod) -> List[DashaPeriod]:
    ...
    for i in range(9):
        ...
        pd_std_years = (DASHA_YEARS[md_planet] * DASHA_YEARS[ad.planet] * DASHA_YEARS[
            pd_planet]) / (TOTAL_DASHA_YEARS ** 2)
...
class ChartData:
    ...
    def get_extended_report(self) -> str:
        ...
        lines.append(
            f"  {'Planet': <10} {'Sthana': >7} {'Dig': >6} {'Kala': >6} {'Chesta': >7} {'Naisa': >7} {'Drik': >6} {'Total': >7} {'Req': >6} {'%': >6} {'Status'}")
        lines.append("   " + "-" * 80)
...
# These rules drive the "Yearly Predictions" tab. 
# They check planetary positions, dignities, transits, and dashas.

PREDICTION_RULES = [
    # ─── CAREER RULES ──────────────────────────────────────────
    {
        "id": "c1", "topic": "career", "severity": "positive", "score": 5,
        "title": "10th Lord Strong & Well-Placed",
        "condition": lambda ctx: _strong(_lord(10, ctx), ctx) and _house(_lord(10, ctx), ctx) in [1, 4, 5, 7, 9, 10, 11],
        "detail": lambda ctx: f"The 10th lord ({_lord(10, ctx)}) is strong ({_dignity(_lord(10, ctx), ctx)}) and placed in a favorable house (H{_house(_lord(10, ctx), ctx)}). This indicates natural career success and authority."
    },
    {
        "id": "c2", "topic": "career", "severity": "warning", "score": -4,
        "title": "10th Lord Debilitated or Afflicted",
        "condition": lambda ctx: _weak(_lord(10, ctx), ctx) or (_combust(_lord(10, ctx), ctx) and not _nb(_lord(10, ctx), ctx)),
        "detail": lambda ctx: f"The 10th lord ({_lord(10, ctx)}) is {_dignity(_lord(10, ctx), ctx)} or combust. Career progress may face obstacles, lack of recognition, or instability. Remedies for the 10th lord are recommended."
    },
    {
        "id": "c3", "topic": "career", "severity": "positive", "score": 6,
        "title": "Jupiter Aspecting 10th House",
        "condition": lambda ctx: "Jupiter" in _benefics_aspect(10, ctx),
        "detail": lambda ctx: "Jupiter’s aspect on the 10th house brings wisdom, ethical growth, and expansion in career. Good time for higher learning or teaching roles."
    },
    {
        "id": "c4", "topic": "career", "severity": "caution", "score": -3,
        "title": "Saturn Transiting 10th House",
        "condition": lambda ctx: _has_transit(ctx) and _transit_house("Saturn", ctx) == 10,
        "detail": lambda ctx: "Transit Saturn in the 10th house brings heavy responsibilities, hard work, and delays. Promotion may be slow but steady if efforts are consistent."
    },
    {
        "id": "c5", "topic": "career", "severity": "positive", "score": 8,
        "title": "Dasha-Karma Yoga Active",
        "condition": lambda ctx: ctx.get("md_planet") == _lord(10, ctx) or ctx.get("ad_planet") == _lord(10, ctx),
        "detail": lambda ctx: f"The running Dasha lord ({ctx.get('md_planet')}/{ctx.get('ad_planet')}) is also the 10th lord. This is a powerful period for career advancement and achieving professional goals."
    },

    # ─── MARRIAGE RULES ────────────────────────────────────────
    {
        "id": "m1", "topic": "marriage", "severity": "positive", "score": 5,
        "title": "7th Lord Strong & Benefic Aspect",
        "condition": lambda ctx: _strong(_lord(7, ctx), ctx) and ("Jupiter" in _benefics_aspect(7, ctx) or "Venus" in _benefics_aspect(7, ctx)),
        "detail": lambda ctx: f"The 7th lord ({_lord(7, ctx)}) is strong and receives benefic aspects. Indicates harmony, happiness, and support from the spouse."
    },
    {
        "id": "m2", "topic": "marriage", "severity": "warning", "score": -5,
        "title": "Mars/Saturn/Rahu in 7th House",
        "condition": lambda ctx: any(p in _malefics_aspect(7, ctx) for p in ["Mars", "Saturn", "Rahu"]),
        "detail": lambda ctx: "Malefic planets in the 7th house can cause delays, conflicts, or dissatisfaction in marriage. Patience and communication are key. Mangal Dosha may be present if Mars is involved."
    },
    {
        "id": "m3", "topic": "marriage", "severity": "positive", "score": 6,
        "title": "Venus Strong or Exalted",
        "condition": lambda ctx: _strong("Venus", ctx) and _house("Venus", ctx) in [1, 4, 5, 7, 9, 10, 11],
        "detail": lambda ctx: "A strong Venus indicates love, romance, and marital bliss. If unmarried, prospects for meeting a compatible partner are high."
    },
    {
        "id": "m4", "topic": "marriage", "severity": "caution", "score": -3,
        "title": "Saturn Transiting 7th House",
        "condition": lambda ctx: _has_transit(ctx) and _transit_house("Saturn", ctx) == 7,
        "detail": lambda ctx: "Saturn transit over the 7th house tests the relationship. It demands maturity, commitment, and realism. Delays in marriage are common during this period."
    },

    # ─── CHILDREN RULES ────────────────────────────────────────
    {
        "id": "ch1", "topic": "children", "severity": "positive", "score": 5,
        "title": "5th Lord Strong & Jupiter Aspect",
        "condition": lambda ctx: _strong(_lord(5, ctx), ctx) and ("Jupiter" in _benefics_aspect(5, ctx)),
        "detail": lambda ctx: "The 5th lord is strong and blessed by Jupiter. Excellent yoga for childbirth, intelligence of children, and happiness from progeny."
    },
    {
        "id": "ch2", "topic": "children", "severity": "warning", "score": -4,
        "title": "5th House Afflicted by Malefics",
        "condition": lambda ctx: any(p in _malefics_aspect(5, ctx) for p in ["Saturn", "Rahu", "Ketu"]),
        "detail": lambda ctx: "Malefic influence on the 5th house can cause delays in conception, anxiety about children, or health issues for progeny. Medical consultation and remedies are advised."
    },
    {
        "id": "ch3", "topic": "children", "severity": "positive", "score": 7,
        "title": "Jupiter Strong in 5th House",
        "condition": lambda ctx: _house("Jupiter", ctx) == 5 and _strong("Jupiter", ctx),
        "detail": lambda ctx: "Jupiter in the 5th house is one of the best placements for children. It indicates wise, healthy, and successful offspring."
    },

    # ─── HEALTH RULES ──────────────────────────────────────────
    {
        "id": "h1", "topic": "health", "severity": "positive", "score": 5,
        "title": "Lagna Lord Strong & Unafflicted",
        "condition": lambda ctx: _strong(_lord(1, ctx), ctx) and _lord(1, ctx) not in _malefics_aspect(1, ctx),
        "detail": lambda ctx: f"The Lagna lord ({_lord(1, ctx)}) is strong and free from malefic aspects. Indicates good vitality, immunity, and general well-being."
    },
    {
        "id": "h2", "topic": "health", "severity": "warning", "score": -6,
        "title": "Saturn/Mars in 6th, 8th, or 12th",
        "condition": lambda ctx: any(_house(p, ctx) in [6, 8, 12] for p in ["Saturn", "Mars"]),
        "detail": lambda ctx: "Malefics in dusthana houses (6, 8, 12) can indicate chronic health issues, accidents, or hospitalization. Regular check-ups and stress management are crucial."
    },
    {
        "id": "h3", "topic": "health", "severity": "caution", "score": -5,
        "title": "Sade Sati Active",
        "condition": lambda ctx: ctx.get("sade_sati_active", False),
        "detail": lambda ctx: f"Sade Sati is active ({ctx.get('sade_sati_phase')}). This period often brings mental stress, physical fatigue, or karmic lessons. Practice meditation and avoid risky activities."
    },
    {
        "id": "h4", "topic": "health", "severity": "positive", "score": 4,
        "title": "Sun Strong in 1st House",
        "condition": lambda ctx: _house("Sun", ctx) == 1 and _strong("Sun", ctx),
        "detail": lambda ctx: "A strong Sun in the Lagna gives excellent vitality, confidence, and resistance to disease."
    }
]
...
def _apply_dasha_boost(fired_rules, topic_lord, md_planet, ad_planet=None, related_planets=None):
    ...
    for r in fired_rules:
        ...
        if rc.get("activation") == "dasha_activated" and md_planet in related and rc["id"] not in boosted:
            ...
            boosted.add(rc["id"])
        elif rc.get("activation") == "dasha_activated" and ad_planet and ad_planet in related and rc[
            "id"] not in boosted:
            old = rc["score"]
...
def _varshphal_themes_v3(
    ...
    themes.append({
        ...
        "modifier": (
            ...
            + ("Strong Muntha lord powerfully supports the year's themes." if muntha_lord_dignity in [
    ...
    themes.append({
        ...
        "nature": "Auspicious" if varsha_lagna_lord_dignity in [
            "Exalted", "Own", "Mool Trikona", "Great Friend"] else "Moderate",
        "calculation": (
    ...
    themes.append({
        ...
        "nature": "Auspicious" if varshesha_dignity in [
            "Exalted", "Own", "Mool Trikona", "Great Friend"] else "Challenging" if varshesha_dignity == "Debilitated" else "Moderate",
        "calculation": (
        ...
        "interpretation": (
            ...
            + ("Productive year." if varshesha_dignity in [
                "Exalted", "Own", "Mool Trikona", "Great Friend"]
    ...
    if transit_planets:
        ...
        jup_nature = "Auspicious" if jup_from_lagna in [1, 2, 5, 9, 11] else "Challenging" if jup_from_lagna in [
            4, 8, 12] else "Moderate"
        ...
        sat_nature = "Challenging" if (
                    sati["active"] or kant["active"] or sat_from_lagna in [4, 7, 8, 10]) else "Positive" if sat_from_lagna in [
            3, 6, 11] else "Moderate"
