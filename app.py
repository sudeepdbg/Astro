...
# ------------------------------------------------------------------
# PERSISTED INPUT COMPONENT (precise time - FIXED)
# ------------------------------------------------------------------
def birth_input_form(key_prefix: str, default_name: str):
    ...
    with city_col:
        city_name = st.text_input("City / Town", st.session_state[ss_city], key=f"{key_prefix}_city",
    ...
    with lat_col:
        lat = st.number_input("Lat", -90.0, 90.0, value=st.session_state[ss_lat], key=f"{key_prefix}_lat_val")
        st.session_state[ss_lat] = lat
    with lon_col:
        lon = st.number_input("Lon", -180.0, 180.0, value=st.session_state[ss_lon], key=f"{key_prefix}_lon_val")
    ...
    if st.button("🔮 Predict", use_container_width=True):
        with st.spinner("Analyzing Dasha, Transit & Varshphal..."):
            ...
            <div class="card">
                ...
                <p><i>{pred.get('overall_summary', '')[:500]}</i></p>
            </div>
            """, unsafe_allow_html=True)
